...
```{r}
grafic <- barplot(table(dades$output, dades$sex), beside = TRUE, col = c("grey", "black"), legend = FALSE)
legend("topright", legend = c("menys risc", "més risc"), fill = c("grey", "black"), x = max(grafic), y = max(grafic) + 120, xpd = TRUE, bty = "n")
```
...
print(paste("Número d'outliers en dones considerant les dades de forma global:", count(dades[c(outliers > 0 & dades$sex=="Dona"),])))
print(paste("Número d'outliers en homes considerant les dades de forma global:", count(dades[c(outliers > 0 & dades$sex=="Home"),])))
...
De forma visual es pot apreciar que no hi ha diferències en quant a la importància de cada variable en la contribució a les 5 primeres components principals.
